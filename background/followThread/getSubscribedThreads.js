...
const SUBSCRIPTION_LINK =
  "https://forums.voz.vn/subscription.php?do=viewsubscription";
...
export default function getSubscribedThreads() {
  return GET(SUBSCRIPTION_LINK, { headers: headerNoCache }).then(html => {
    ...
    if ($form.length === 0) {
      if (
        $$.find(
          "*:contains('You are not logged in or you do not have permission to access this page')"
        ).length > 0
      ) {
