...
class QuoteBackground {
  constructor() {
    ...
    chrome.runtime.onMessage.addListener(request => {
      if (request.continueNotify && request.continueNotify === true)
        this.hasNotify = false;
    });
  }
    ...
    getChromeLocalStore(["quotes"]).then(result => {
      ...
      if (!existQuotes || existQuotes.length === 0) {
        ...
        quotes.forEach(quote => {
          const existQuote = existQuotes.find(
            eq => eq.post.id == quote.post.id
          );
          ...
          () => {
            try {
              if (hasNotSeen.length === 1) {
                const handler = () => {
                  window.open(
                    `https://forums.voz.vn/showthread.php?p=${
                      hasNotSeen[0].post.id
                    }#post${hasNotSeen[0].post.id}`,
                    "_blank"
                  );
  ...
  checkGetQuotes() {
    ...
    getChromeLocalStore(["authInfo", "settings"]).then(
      ({ settings, authInfo }) => {
