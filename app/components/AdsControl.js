...
function removeAds() {
  $('head')
    .append(`<style>[id^=div-gpt-ad],[id^=google_ads_div],.middleads,[id^=ads_zone], tbody > tr > td:nth-child(2)[width="160"]{
  display: none}
}</style>`);
  if (
    $('.middleads+table > tbody > tr > td:eq(1) [id^=div-gpt-ad]').length > 0
  ) {
...
export function AdsControl() {
  ...
  getLocalSettings().then(settings => {
    ...
    localStorage.setItem(
      'adsRemove',
      settings.adsRemove === true ? 'true' : 'false'
    );
  });
}
